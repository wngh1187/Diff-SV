...
from .noise_aug import Noises

class DataPreprocessor:
    def __init__(self, path_musan, path_nonspeech, path_vox1_test):
        self.path_musan = path_musan
        self.path_nonspeech = path_nonspeech
    ...
    def check_environment(self):
        ...
        if not os.path.exists(self.path_vox1_test_noise):		
            self.noises = Noises(self.path_musan_split + '/test', self.path_nonspeech)
        ...
        for root, _, files in tqdm(os.walk(self.path_vox1_test)):
            for file in files:
                if '.wav' in file:
                    ...
                    self._inject_noise(path, 'nonspeech', 0)
                    self._inject_noise(path, 'nonspeech', 5)
                    self._inject_noise(path, 'nonspeech', 10)
                    self._inject_noise(path, 'nonspeech', 15)
                    self._inject_noise(path, 'nonspeech', 20)

    def _inject_noise(self, path, category, snr):
        data, sr = sf.read(path)
        self.noises.Category = [category]
        self.noises.SNR[category] = (snr, snr)
        destination = path.replace(
            self.dir_vox1_test, f'{self.dir_vox1_test}_noise/{category}_{snr}')
        os.makedirs(os.path.dirname(destination), exist_ok=True)
        data = self.noises(data, [category])
